...
function mockApi(actionFixture = action, plantFixture = plant, simulator = true) {
  ...
  return vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
    ...
    if (url.endsWith("/health")) return new Response(JSON.stringify({ status: "ready", version: "0.8.5", database: "ready", simulator, plant_doctor_configured: true, home_assistant_notifications_enabled: !simulator, stale_sensor_hours: 72 }), { status: 200 });
...
describe("portal", () => {
  ...
  it("opens the linked plant directly from a Home Assistant notification", async () => {
    ...
    expect(await screen.findByRole("dialog", { name: "Golden Pothos" })).toBeInTheDocument();
